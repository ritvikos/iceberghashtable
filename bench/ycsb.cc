...
void ycsb_load_run_randint(int index_type, int wl, int kt, int ap,
	...
	if (ap == UNIFORM) {
		if (kt == RANDINT_KEY && wl == WORKLOAD_A) {
			init_file = "/mnt/nvme3/RECIPE/index-microbench/"
				    "workloads/loada_unif_int.dat";
			txn_file = "/mnt/nvme3/RECIPE/index-microbench/"
				   "workloads/txnsa_unif_int.dat";
		} else if (kt == RANDINT_KEY && wl == WORKLOAD_B) {
			init_file = "/mnt/nvme3/RECIPE/index-microbench/"
				    "workloads/loadb_unif_int.dat";
			txn_file = "/mnt/nvme3/RECIPE/index-microbench/"
				   "workloads/txnsb_unif_int.dat";
		} else if (kt == RANDINT_KEY && wl == WORKLOAD_C) {
			init_file = "/mnt/nvme3/RECIPE/index-microbench/"
				    "workloads/loadc_unif_int.dat";
			txn_file = "/mnt/nvme3/RECIPE/index-microbench/"
				   "workloads/txnsc_unif_int.dat";
		} else if (kt == RANDINT_KEY && wl == WORKLOAD_D) {
			init_file = "/mnt/nvme3/RECIPE/index-microbench/"
				    "workloads/loadd_unif_int.dat";
			txn_file = "/mnt/nvme3/RECIPE/index-microbench/"
				   "workloads/txnsd_unif_int.dat";
		} else if (kt == RANDINT_KEY && wl == WORKLOAD_E) {
			init_file = "/mnt/nvme3/RECIPE/index-microbench/"
				    "workloads/loade_unif_int.dat";
			txn_file = "/mnt/nvme3/RECIPE/index-microbench/"
				   "workloads/txnse_unif_int.dat";
		}
	} else {
		if (kt == RANDINT_KEY && wl == WORKLOAD_A) {
			init_file = "/mnt/nvme3/RECIPE/index-microbench/"
	...
	if (index_type == TYPE_ICEBERG) {
...

		{
			...
			auto func = [&]() {
				...
				uint64_t start_key = LOAD_SIZE / num_thread *
						     (uint64_t)thread_id;
				uint64_t end_key =
				    start_key + LOAD_SIZE / num_thread;

				for (uint64_t i = start_key; i < end_key; i++) {
					if (!iceberg_insert(
						tds[thread_id].ht, init_keys[i],
...

		{
			...
			auto func = [&]() {
				...
				uint64_t start_key = txn_count / num_thread *
						     (uint64_t)thread_id;
				uint64_t end_key =
				    start_key + txn_count / num_thread;
#ifdef LATENCY
					...
					if (ops[i] == OP_INSERT) {
...
#ifdef LATENCY
						std::chrono::high_resolution_clock::
						    time_point t2 = std::chrono::
							high_resolution_clock::
							    now();
						insert_times.emplace_back(
						...
						std::chrono::high_resolution_clock::
							...
							high_resolution_clock::
							    now();
#endif
						auto ret = iceberg_get_value(
						    tds[thread_id].ht, keys[i],
						    &val, thread_id);
						if (val != keys[i]) {
							std::cout
							    << "[ICEBERG] "
...
int main(int argc, char **argv) {
	if (argc != 6) {
		std::cout
		    << "Usage: ./ycsb [index type] [ycsb workload type] [key "
	...
	if (kt != STRING_KEY) {
		...
		ycsb_load_run_randint(index_type, wl, kt, ap, num_thread,
				      init_keys, keys, ranges, ops);
	}
	/*
	else {
	    ...
	    ycsb_load_run_string(index_type, wl, kt, ap, num_thread, init_keys,
	keys, ranges, ops);
	}
	*/
